...
def apply_pad(p: object, move: object, problem: object) -> object:
    if move == 'A':
        if problem == 'p1':
            return p, getPad1(p)
        else:
...
def solve1(code,pads):
    ...
    while Q:
        ...
        for move in ['^', '<', 'v', '>', 'A']:
            new_p1 = p1
            new_out = out
            new_p1, output = apply_pad(p1, move, 'p1')
    ...
    while Q:
        ...
        for move in ['^', '<', 'v', '>', 'A']:
            ...
            for i,p in reversed(list(enumerate(ps))):
                new_ps[i], new_move = apply_pad(ps[i], new_move, 'p2')
                if new_move is None:
                    break
                else:
            ...
            if new_move is not None:
                new_p1, output = apply_pad(p1, new_move, 'p2')
                if output == ch:
                    return path+move
...
def cost2(ch, prev_move, pads):
    ...
    if pads==0:
        ...
        while Q:
            ...
            SEEN[seen_key] = d
            for move in ['^', '<', 'v', '>', 'A']:
                new_p, output = apply_pad(p, move, 'p2')
